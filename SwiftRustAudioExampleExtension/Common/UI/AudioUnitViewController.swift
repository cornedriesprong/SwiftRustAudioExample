...
private let log = Logger(subsystem: "cp3.SwiftRustAudioExampleExtension", category: "AudioUnitViewController")

public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<SwiftRustAudioExampleExtensionMainView>?
	...
	*/
    ...
    public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
        audioUnit = try SwiftRustAudioExampleExtensionAudioUnit(componentDescription: componentDescription, options: [])
        
        guard let audioUnit = self.audioUnit as? SwiftRustAudioExampleExtensionAudioUnit else {
            log.error("Unable to create SwiftRustAudioExampleExtensionAudioUnit")
            return audioUnit!
        }
        ...
        audioUnit.setupParameterTree(SwiftRustAudioExampleExtensionParameterSpecs.createAUParameterTree())
    ...
    private func configureSwiftUIView(audioUnit: AUAudioUnit) {
        ...
        let content = SwiftRustAudioExampleExtensionMainView(parameterTree: observableParameterTree)
